...
# 讀取數據
data = pd.read_csv('penergys/penergy10000.csv')
...
for x in State:
    ...
    if not matched_row.empty:
        original_y.append(matched_row['Potential Energy'].values[0])
    else:
...
result.to_csv(f'pfit/{formula}.csv', index=False)
print("結果已儲存成功")
