...
# 忽略第一筆數據
x_data = data['Matrix size'].values[3:]  # 忽略第一筆
y_data = data['Coefficient'].values[3:]  # 忽略第一筆
...
# 原始數據的對應 y 值
original_y = data['Coefficient'].values[3:]  # 忽略第一筆數據
...
# 建立結果 DataFrame
result = pd.DataFrame({
    'Matrix size': data['Matrix size'].values[3:],  # 忽略第一筆數據
...
#result.to_csv(f'allpfit/{formula}.csv', index=False)
#print("結果已儲存成功")