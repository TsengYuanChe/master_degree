...
# 讀取數據
remove = 1000
file_path = f'fiteigenvalues/eigenvalue2/eigenvalue2_data_{remove}.csv'
data = pd.read_csv(file_path)

# 過濾掉沒有 Log(Adjusted Eigenvalue) 的數據
filtered_data = data.dropna(subset=['Log(Adjusted Eigenvalue)'])

filtered_data = filtered_data[filtered_data['Matrix size'] > remove]
...
try:
    ...
    # 儲存與顯示圖表
    output_file = f'fiteigenvalues/eigenvalue2/fitting_result{remove}.png'
