...
# 讀取數據
file_path = 'findev/eigenvalues_data.csv'
data = pd.read_csv(file_path)

# 過濾掉沒有 Log(Adjusted Eigenvalue) 的數據
data = data.dropna(subset=['Log(Adjusted Eigenvalue 1)'])
filtered_data = data.dropna(subset=['Log(Adjusted Eigenvalue 2)'])
...
# 原始數據
plt.scatter(x_data, y_data[0], label=f'Original Data for Eigenvalue {i+1}', color='blue', s=10)
plt.scatter(x_data, y_data[1], label=f'Original Data for Eigenvalue {i+1}', color='red', s=10)
...
# 儲存與顯示圖表
output_file = 'findev/fitting_result.png'
